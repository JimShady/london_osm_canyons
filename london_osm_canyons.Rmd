...
mapview(temp_raster, alpha.regions = 0.4, map.types = 'OpenStreetMap.BlackAndWhite') %>% addFeatures(st_transform(roads[roads$osm_id == osm_id_to_examine,]$area,4326))
...
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= -22.5   & roads$bearing <= 22.5,]$classification  <- 1
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= 157.5   & roads$bearing <= 180,]$classification   <- 1
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing <= -157.5  & roads$bearing > -180,]$classification   <- 1
```
...
```{r east_west}
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= 67.5    & roads$bearing <= 112.5,]$classification <- 2
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= -112.5  & roads$bearing <= -67.5,]$classification <- 2
```
...
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= 112.5    & roads$bearing <= 157.5,]$classification  <- 3
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing <= -22.5  & roads$bearing   >= -67.5,]$classification  <- 3
```
...
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing >= 22.5    & roads$bearing  <= 67.5,]$classification   <- 4
roads[roads$non_zero_means > 10 & !is.na(roads$non_zero_means) & roads$bearing <= -112.5  & roads$bearing >= -157.5,]$classification  <- 4
```
...
roads[roads$non_zero_means <= 1 & !is.na(roads$non_zero_means) ,]$classification   <- 5
...
roads[roads$non_zero_means > 1 & roads$non_zero_means <= 10 & !is.na(roads$non_zero_means) ,]$classification   <- 6
...
Now write out the classified roads as a geojson

```{r write_out}
st_write(roads, 'final_roads_london.geojson')
```

Roads that are covered by the height raster
...
```{r}
##  Create cuts:
x_c <- cut(roads[!is.na(roads$non_zero_means) & roads$non_zero_means != 0,]$non_zero_means, 20)
y_c <- cut(roads[!is.na(roads$non_zero_means) & roads$non_zero_means != 0,]$total_width, 20)
