...
roads[roads$half_of_one == 'Yes' & !is.na(roads$half_of_one),]$height_width_ratio_excl_zeros <- roads[roads$half_of_one == 'Yes' & !is.na(roads$half_of_one),]$non_zero_means / 
                                                                                                (roads[roads$half_of_one == 'Yes' & !is.na(roads$half_of_one),]$total_width * 2)
...
Need to classify the roads as canyons and directions for modelling. If the height is above 10 metres they wil l be classified as a canyon of a specific direction as such.
...
roads[roads$non_zero_means > 10 & roads$non_zero_means <= 20 & !is.na(roads$non_zero_means) & roads$bearing >= 22.5    & roads$bearing  <= 67.5,]$classification   <- '4b'
roads[roads$non_zero_means > 10 & roads$non_zero_means <= 20 & !is.na(roads$non_zero_means) & roads$bearing <= -112.5  & roads$bearing >= -157.5,]$classification  <- '4b'
...
```{r get_summary_width_height}



```

Roads that are covered by the height raster shown

```{r road_height_and_width_plot}
values(london_raster)[values(london_raster) == 0] = NA

plot(london_raster, col='red')
plot(st_geometry(st_transform(roads, crs(london_raster)@projargs)), add=T, color=rgb(0,0,0,alpha=0.1))
plot(london_raster, col='red', add=T)

```


```{r}
kable(aggregate(data = roads, cbind(non_zero_means, total_width) ~ classification, FUN=summary))

kable(aggregate(data = roads, cbind(lanes, total_width) ~ type, FUN=mean))
```
...
There are `r nrow(roads)` in the dataset.

```{r}
##  Create height cuts:
roads$height_cut <- NA

breaks <- c(0,5,10,20,30,40,50,500)

roads$height_cut <-  cut(roads$non_zero_means, breaks)

ggplot(roads[!is.na(roads$height_cut),], aes(x=total_width)) + geom_histogram(fill='red') + facet_grid(.~height_cut)

```

If we are going to have more than one type of street canyon, they would be as follows:

Current number of canyons: `r nrow(roads[roads$classification %in% c(1,2,3,4),])`

Change to:

Low canyons `r nrow(roads[roads$non_zero_means > 10 & roads$non_zero_means <= 20 & !is.na(roads$non_zero_means),])`

Medium canyons `r nrow(roads[roads$non_zero_means > 20 & roads$non_zero_means <= 30 & !is.na(roads$non_zero_means),])`

High canyons `r nrow(roads[roads$non_zero_means > 30 & !is.na(roads$non_zero_means),])`

```{r }

nrow(roads[roads$total_width > 10 & roads$total_width < 20 & !is.na(roads$total_width) & roads$non_zero_means >= 10 & roads$non_zero_means <= 20 & !is.na(roads$non_zero_means),]) #low canyon, 4,251


nrow(roads[roads$total_width > 10 & roads$total_width < 20 & !is.na(roads$total_width) & roads$non_zero_means > 20 & roads$non_zero_means <= 30 & !is.na(roads$non_zero_means),]) #medium canyon, 1,121

nrow(roads[roads$total_width > 10 & roads$total_width < 20 & !is.na(roads$total_width) & roads$non_zero_means > 30 & !is.na(roads$non_zero_means),]) #high canyons 356
