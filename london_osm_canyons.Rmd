...
values(london_raster)[is.na(values(london_raster))] <- 0

roads$mean_non_zero_height  <- extract(london_raster, road_polygons, weights=TRUE, na.rm = T, fun=mean)[,1]

roads$percent_zero          <- 
  
roads$mean_height           <- extract(london_raster, road_polygons, weights=TRUE, na.rm = F, fun=mean)[,1]
