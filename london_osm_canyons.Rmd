...
Now going to classify the remaining roads.
```{r rest_of_the_roads}

roads[is.na(roads$classification) & roads$type %in% c('motorway', 'motorway_link'),'classification'] <- 5

roads[is.na(roads$classification) & roads$type %in% c('primary', 'primary_link'),'classification'] <- 6

roads[is.na(roads$classification) & roads$type %in% c('secondary', 'tertiary'),'classification'] <- 6

roads[is.na(roads$classification) & roads$type %in% c('trunk', 'trunk_link'),'classification'] <- 6
...
extra_roads$osm_id      <- as.numeric(as.character(residential$osm_id))
...
extra_roads$half_of_one                     <- as.character(NA)
...
extra_roads$non_zero_means                  <- as.numeric(NA)
extra_roads$height_width_ratio_excl_zeros   <- as.numeric(NA)
extra_roads$classification                  <- as.numeric(6)
...
Want to keep file size down, so round some things up

```{r}

roads$non_zero_means <- round(roads$non_zero_means,1)
roads$oneway         <- as.character(roads$oneway)
roads[roads$oneway == 'yes' & !is.na(roads$oneway),'oneway'] <- 'Y'
roads$oneway          <- as.factor(roads$oneway)
roads$lanes           <- as.factor(roads$lanes)
```

Another round of removing duplicates.
...
st_write(roads[,c('osm_id', 'oneway', 'lanes', 'type', 'half_of_one', 'width', 'total_width', "non_zero_means", "classification")], 
         dsn='final_roads_london.gpkg', delete_dsn = T)
```
