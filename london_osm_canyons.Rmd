...
roads[roads$canyon_height == '-Inf','canyon_height'] <- NA
print('done')
...
```{r}
london_kernels_in_use <- aggregate(data = roads[!is.na(roads$height_cat),], cbind(width, total_width, canyon_height) ~ height_cat + width_cat + orientation_classification, FUN=mean )

road_types <- merge(road_types, london_kernels_in_use, by.x = c('width_cat', 'height_cat', 'orientation'), by.y = c('width_cat', 'height_cat', 'orientation_classification'), all.x=T)
...
sum_of_weights_per_dotref <- aggregate(data=dotref_roads, length ~ DotRef+road_type, FUN=sum)
