...
```{r remove problem roads and make buffers}
...
test_raster <- extent(st_bbox(roads[10,])$xmin-40, st_bbox(roads[10,])$xmax+40, 
                      st_bbox(roads[10,])$ymin-40, st_bbox(roads[10,])$ymax+40)
test_raster <- crop(london_raster, test_raster)

mapview(test_raster, alpha.regions = 0.4, map.types = 'OpenStreetMap.BlackAndWhite') %>% addFeatures(st_transform(roads[10,]$area,4326))
```

These go into new columns in the 'roads' data.
...
for (i in 1:nrow(roads)) {
  ...
  polygon <- road_polygons[i,]
  
  roads[i,'cell_count']                   <- length(extract(london_raster, polygon, weights=T, na.rm=F)[[1]][,1])
  
  roads[i,'weighted_mean']                <- extract(london_raster, polygon, weights=T, na.rm=F, fun=mean)[1]
  
  roads[i,'non_zero_non_weighted_mean']   <- mean(extract(london_raster, polygon, weights=T, na.rm=F)[[1]][extract(london_raster, polygon, weights=T, na.rm=F)[[1]][,1] != 0,1])
  
  roads[i,'zero_cells_count']             <- length(extract(london_raster, polygon, weights=T, na.rm=F)[[1]][extract(london_raster, polygon, weights=T, na.rm=F)[[1]][,1] == 0,1])
