...
roads$canyon_height                            <- sapply(extracted, FUN = function(x)(sum(x[x[,1]>0,1] * (x[x[,1]>0,2] / sum(x[x[,1]>0,2])))))
...
Need to classify the roads as canyons and directions for modelling. Use the street canyon categories Sean has made kernels for.
...
#roads <- st_read('roads_after_height_extraction.geojson')
```
...
roads[roads$bearing >= -22.5   & roads$bearing <= 22.5,]$orientation_classification   <- 'NS'
roads[roads$bearing >= 157.5   & roads$bearing <= 180, ]$orientation_classification   <- 'NS'
roads[roads$bearing <= -157.5  & roads$bearing > -180, ]$orientation_classification   <- 'NS'

roads[roads$bearing >= 67.5    & roads$bearing <= 112.5,]$orientation_classification  <- 'EW'
roads[roads$bearing >= -112.5  & roads$bearing <= -67.5,]$orientation_classification  <- 'EW'

roads[roads$bearing >= 112.5   & roads$bearing <= 157.5,]$orientation_classification  <- 'SENW'
roads[roads$bearing <= -22.5   & roads$bearing >= -67.5,]$orientation_classification  <- 'SENW'

roads[roads$bearing >= 22.5    & roads$bearing <= 67.5,]$orientation_classification   <- 'SWNE'
roads[roads$bearing <= -112.5  & roads$bearing >= -157.5,]$orientation_classification <- 'SWNE'
...
We have some roads remaining that are not classified. Do these manually as below.

```{r rest_of_the_roads}

# Motorway as open road.
roads[is.na(roads$road_type) & roads$type %in% c('motorway', 'motorway_link'),'road_type'] <- 73

# Rest as typical
roads[is.na(roads$road_type),'road_type'] <- 74
...
dotref_roads  <- roads[!is.na(roads$DotRef),c('DotRef', 'road_type')] # bearing could be road length
...
dotref_roads$weight <- dotref_roads$road_type * dotref_roads$length

sum_of_weights_per_dotref <- aggregate(data=dotref_roads, weight ~ DotRef, FUN=sum)

dotref_roads <- merge(dotref_roads, sum_of_weights_per_dotref, by.x = 'DotRef', by.y = 'DotRef')

dotref_roads$new_road_type <- dotref_roads$weight.x / dotref_roads$weight.y

dotref_roads$new_road_type <- dotref_roads$new_road_type * dotref_roads$road_type

dotref_roads <- aggregate(data=dotref_roads, new_road_type ~ DotRef, FUN=sum)

dotref_roads$new_road_type <- round(dotref_roads$new_road_type, 0)

# Now import Gregor file and update with new types

dotref_points <- read.csv('Oscar_DOT_Toid_10m.csv')

dotref_points <- merge(dotref_points, dotref_roads, by.x = 'Dot_ref', by.y = 'DotRef', all.x=T)
