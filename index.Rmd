...
```{r}
#roads   <- opq(bbox = c(min_x, min_y, max_x, max_y)) %>% 
...
```{r}
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), 'test.html')) })
```